...
/datum/proc/_AddComponent(list/raw_args, source)
	...
	// 	if(dupe_mode != COMPONENT_DUPE_ALLOWED && dupe_mode != COMPONENT_DUPE_SELECTIVE && dupe_mode != COMPONENT_DUPE_SOURCES) - monkestation edit original
	...
	// MONKESTATION REMOVAL START - fix COMPONENT_DUPE_SOURCES duplicating components instead of adding to the list of sources
