...
const SORT_OPTIONS = ['distance', 'ijob', 'name', 'area'];
...
const NtosLifelineContent = () => {
  ...
  const [sortAsc, setSortAscLocal] = useLocalState<boolean>(
    'sortAsc',
    settings.sortAsc,
  );
    ...
    <NtosWindow.Content scrollable minHeight="540px">
      <Stack vertical>
        <Stack.Item style={{ display: 'flex' }}>
          ...
          <Button.Checkbox style={{ float: 'right' }}>
...
const CrewTab = (props: { object: CrewSensor }) => {
  ...
  return (
    ...
    >
      ...
      />
      <Box color={jobToColor(object.ijob)} style={{ display: 'flex' }}>
        <Box>
          <Icon
            name={JOB2ICON[object.job] || 'question'}
            width="25px"
            style={{ float: 'left', padding: '4px' }}
          />
        </Box>
        <Box>
          {object.name} ({object.jobTitle})
          <br />
          {object.dist > 0 && (
            <Icon
              mr={0.2}
