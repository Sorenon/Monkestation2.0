...
/obj/machinery/microwave/MouseDrop_T(mob/living/target, mob/living/user)
	if(target.buckled || target.has_buckled_mobs())
		return
	put_in_microwave(target, user)

/obj/machinery/microwave/proc/put_in_microwave(mob/living/target, mob/living/user)
	if(!istype(target))
		return
...
/obj/machinery/microwave/proc/attempt_escape(mob/user)
	eject(force=TRUE)
